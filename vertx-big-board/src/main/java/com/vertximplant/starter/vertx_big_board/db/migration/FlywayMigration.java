...
import com.vertximplant.starter.vertx_big_board.config.MySqlDbConfig;
...
public class FlywayMigration {
  ...
  public static Future<Void> migrate(Vertx vertx, DbConfig dbConfig, MySqlDbConfig mySqlDbConfig) {
    // Running on a blocking Thread
    LOG.debug("DB Config: {}", dbConfig);
    return vertx.<Void>executeBlocking(promise -> {
      execute(dbConfig);
//      executeMysql(mySqlDbConfig);
      promise.complete();
  ...
  private static void executeMysql(MySqlDbConfig dbConfig) {
    final String jdbcUrl = String.format("jdbc:mysql://%s:%d/%s", dbConfig.getHost(),
      dbConfig.getPort(), dbConfig.getDatabase());
    LOG.debug("Migrating DB schema using jdbc url: {}", jdbcUrl);
