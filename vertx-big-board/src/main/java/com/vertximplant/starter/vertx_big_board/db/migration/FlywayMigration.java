...
public class FlywayMigration {
  ...
  private static void execute(DbConfig dbConfig) {
    ...
    final Flyway flyway = Flyway.configure().dataSource(jdbcUrl, dbConfig.getUser(), dbConfig.getPassword())
      .schemas("broker")
  ...
  private static String printMigrtions(final MigrationInfo[] pendingMigrations) {
    ...
    return Arrays.stream(pendingMigrations).map(each -> each.getVersion()+" - "+each.getDescription())
