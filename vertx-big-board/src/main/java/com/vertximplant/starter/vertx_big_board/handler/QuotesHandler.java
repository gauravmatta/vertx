...
public class QuotesHandler {
  ...
  public void handle(RoutingContext routingContext) {
    ...
    if(quote.isEmpty()){
      ...
      handleErrorResponse("t_test",routingContext,"userId",new Failure(500,response),stopwatch);
    return;
    }
  ...
  private void handleSuccessResponse(String transid,RoutingContext routingContext, Quote quote, Stopwatch stopwatch){
    responseBuilder.sendResponse(routingContext.request(),SC_OK,gsonToString(quote),responseBuilder.buildResponseHeaders(transid),stopwatch);
  }

  private void handleErrorResponse(String transid, RoutingContext routingContext,String identifier,Throwable throwable,Stopwatch stopwatch){
    responseBuilder.exceptionResponseHandler(transid,responseBuilder.buildLogEndIdentifier("user_id",identifier),"Get Quotes Information",throwable,routingContext.request(),((Failure) throwable).getStatusMsg(),stopwatch);
  }
